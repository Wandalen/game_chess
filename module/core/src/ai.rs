...
//! Wraps tanton bots in dyn traits. Serializes algorithms.
//!
...
macro_rules! implement_algorithm_trait {
  ($name:ident, $searcher:ty, $short_name:expr) => {
    ...
    impl Algorithm for $name {
      ...
      fn best_move(&self, board: Board, depth: u16) -> Move {
        <$searcher>::best_move(board.tanton_board, depth)
      }
    }
  };
}

implement_algorithm_trait!(MinMaxAlgorithm, tanton::bots::ParallelMiniMaxSearcher, "min_max");
implement_algorithm_trait!(IterativeAlgorithm, tanton::bots::IterativeSearcher, "iterative");
implement_algorithm_trait!(RandomAlgorithm, tanton::bots::RandomBot, "random");
...
impl Engine {
  ...
  ///
  pub fn new(name: String) -> Result<Self, CreationError> {
    Self::new_with_depth(name, 5)
  }
