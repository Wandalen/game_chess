...
pub use tanton::{
  ...
  core::piece_move::BitMove as Move, //https://docs.rs/tanton/latest/tanton/core/piece_move/index.html
...
Board
  tanton_board : tanton::Board

HistoryEntry
  fen : String
  last_move : Move https://docs.rs/tanton/latest/tanton/core/piece_move/struct.BitMove.html
...
impl Board {
  ...
  pub fn default() -> Self {
    Self {
      tanton_board: tanton::Board::start_pos(),
    }
  }
  ...
  pub fn from_fen(fen: &Fen) -> Self {
    match tanton::Board::from_fen(fen) {
      Ok(tanton_board) => Self { tanton_board },
      _ => Self::default(),
    }
  }
  ...
  pub fn make_move(&mut self, uci_move: UCI) -> Option<Self> {
    let mut tanton_board: tanton::Board = self.tanton_board.clone();
    let result = tanton_board.apply_uci_move(&uci_move.0);
    if result {
      Some(Self { tanton_board })
    } else {
      None
    }
  }
  ...
  pub fn move_is_valid(&self, uci_move: UCI) -> bool {
    match self.move_from_uci(uci_move) {
      Some(m) => self.tanton_board.pseudo_legal_move(m) && self.tanton_board.legal_move(m),
      _ => false,
    }
  }

  ///
  /// Looks for a valid [Move](https://docs.rs/tanton/latest/tanton/core/piece_move/struct.BitMove.html) from move in UCI format.
  ///
  pub fn move_from_uci(&self, uci_move: UCI) -> Option<Move> {
    let all_moves: MoveList = self.tanton_board.generate_moves();
  ...
  pub fn make_move_ai(&mut self) {
    let turn = self.tanton_board.turn();
    ...
    let best_move = self
      ...
      .into_iter()
      .map(|m| {
        self.tanton_board.apply_move(m);
        let score = tanton::tools::eval::Eval::eval_low(&self.tanton_board);
        self.tanton_board.undo_move();
        (m, score)
      })
      .max_by(|(_, a), (_, b)| if turn == Player::Black { a.cmp(b) } else { b.cmp(a) })
      .unwrap()
      .0;

    self.tanton_board.apply_move(best_move);
  }
  ...
  pub fn piece_at(&self, sq: u8) -> Piece {
    self.tanton_board.piece_at_sq(Cell(sq))
  }
  ...
  pub fn score(&self) -> i32 {
    tanton::tools::eval::Eval::eval_low(&self.tanton_board)
    //0
    /* ttt : implement me */
  }
  ...
  pub fn to_pretty_string(&self) -> String {
    let mut s = String::with_capacity(tanton::core::masks::SQ_CNT * 2 + 40);
    let mut rank = 8;

    for sq in tanton::core::masks::SQ_DISPLAY_ORDER.iter() {
      if sq % 8 == 0 {
      ...
      let op = self.tanton_board.get_piece_locations().piece_at(tanton::SQ(*sq));
  ...
  pub fn to_fen(&self) -> Fen {
    Fen::from(self.tanton_board.fen())
  }
}
...
/// Field `last_move` information about last [Move]https://docs.rs/tanton/latest/tanton/core/piece_move/struct.BitMove.html)
///
...
///
/// Serialize [Move](https://docs.rs/tanton/latest/tanton/core/piece_move/struct.BitMove.html)
///

pub fn move_ser<S: Serializer>(m: &Move, s: S) -> Result<S::Ok, S::Error> {
  s.serialize_u16(m.get_raw())
}

///
/// Deserialize [Move](https://docs.rs/tanton/latest/tanton/core/piece_move/struct.BitMove.html)
///
...
impl Game {
  ...
  pub fn moves_list(&self) -> MoveList {
    self.board.tanton_board.generate_moves()
  }
  ...
  pub fn make_move_ai(&mut self) {
    match &self.ai {
      Some(engine) => self.board.tanton_board.apply_move(engine.best_move(self.board.clone())),
  ...
  ///
  /// Returns last move as [Move](https://docs.rs/tanton/latest/tanton/core/piece_move/struct.BitMove.html))
  /// Returns None if there are no moves.
  ///
  ...
  ///
  pub fn board_state_printable(&self) -> String {
  ...
  ///
  pub fn move_is_valid(&self, uci_move: UCI) -> bool {
    self.board.move_is_valid(uci_move)
  }
}
...
#[cfg(target_arch = "wasm32")]
pub fn get_unix_timestamp(_start: Option<js_sys::Date>) -> u64 {
