...
fn main()
{
  App::build()
    ...
    .add_system_set_to_stage(CoreStage::PostUpdate, SystemSet::new().with_system(cursor_system.system()))
    .run();
}

//

fn setup(mut commands : Commands) { commands.spawn().insert_bundle(OrthographicCameraBundle::new_2d()); }
